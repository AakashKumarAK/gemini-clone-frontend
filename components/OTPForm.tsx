...
export default function OTPForm() {
  const { register, handleSubmit, setValue, watch } = useForm<FormValues>({
    ...
    defaultValues: { countryCode: "+91", phone: "", otp: "" }, // <-- add otp
  });
    ...
    <div className="max-w-md mx-auto bg-white dark:bg-slate-900 p-6 rounded shadow">
      {step === "phone" ? (
        <form onSubmit={handleSubmit(onSend)} className="space-y-6">
          <div>
            <label className="text-sm block mb-1 text-slate-700 dark:text-slate-200">
              Country
            </label>
            <CountrySelect
        ...
        <form onSubmit={handleSubmit(onVerify)} className="space-y-4">
          <div>
            ...
            <input
              {...register("otp")} // <-- type-safe, no 'any'
              className="w-full border rounded px-3 py-2 text-slate-800"
              placeholder="123456"
            />
          </div>
          <div className="flex gap-2">
            <button className="bg-green-600 text-white px-4 py-2 rounded">Verify</button>
            <button
              type="button"
